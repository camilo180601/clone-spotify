...
const SongControl = ({ audio, nextSong }) => {
...
const Player = () => {
    ...
    const nextSong = () => {
        ...
        if (song) {
            ...
            const src = `/music/${playlist?.id}/${nextSongId}.mp3`;
            ...
            fetch(`/api/get-info-playlist.json?id=${playlist?.id}`)
                ...
                .then(data => {
                    ...
                    setIsPlaying(true)
                    setCurrentMusic({ songs, playlist, song: songs[nextSongIndex]})
                })
    ...
    const previousSong = () => {
        ...
        if (song) {
            ...
            const previousSongId = songs[previousSongIndex].id;
            const src = `/music/${playlist?.id}/${previousSongId}.mp3`;
            ...
            fetch(`/api/get-info-playlist.json?id=${playlist?.id}`)
                ...
                .then(data => {
                    const { songs, playlist } = data
                    setIsPlaying(true)
                    setCurrentMusic({ songs, playlist, song: songs[previousSongIndex]})
                })
        ...
        <div className="flex flex-row justify-between w-full px-1 z-50">
            ...
            <div className='grid place-content-center gap-4 flex-1'>
                <div className='flex justify-center flex-col items-center'>
                    <div>
                        <button className='p-2 mr-3 mt-1'>
                            <IconPlayerSkipBackFilled color='white' onClick={previousSong}/>
                        </button>
                        <button className='bg-white rounded-full p-2' onClick={handleClick}>
                            {isPlaying ? <Pause /> : <Play />}
                        </button>
                        <button className='p-2 ml-3 mt-1' onClick={nextSong}>
                            <IconPlayerSkipForwardFilled color='white' />
                        </button>
                    </div>
                    <SongControl audio={audioRef} nextSong={nextSong}/>
                    <audio ref={audioRef} />
                </div>
            </div>
